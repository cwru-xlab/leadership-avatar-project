...
- A compact `ScenarioSnapshotStrip` above the score cards (READY and FAILED only) reads exclusively from `report.scenario` (the DTO's run-time snapshot) — scenario name, character `{name, role}` chips, and a criteria-presence dot — labelled "This scenario as it was when you practiced."
